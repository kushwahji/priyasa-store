...
test.describe('PRIYASA storefront smoke', () => {
  ...
  test('header search works', async ({ page }) => {
    await page.goto('/'); const search = page.locator('header input[aria-label="Search products"]').first();
  ...
  test('authenticated checkout loads saved addresses through the documented contract', async ({ page }) => {
    await requireSession(page); const addresses = page.waitForResponse(r => r.url().includes('/api/priyasa/storefront/addresses') && r.request().method() === 'GET');
