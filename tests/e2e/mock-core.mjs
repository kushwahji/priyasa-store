...
const state = { otpRequestId: 'otp-test-1', accessToken: 'e2e-access-token', orderId: '1001', addressId: 'addr-1' };
...
const server = http.createServer(async (req,res) => {
  ...
  if(req.method==='GET' && path==='/storefront/home') return json(res,200,{success:true,data:{version:1,currency:'INR',locale:'en-IN',sections:[
    {id:1,key:'home-hero',type:'hero_slider',sort_order:10,is_active:true,content:{autoplay:false,items:[{id:'hero-1',title:'Style It Your Way',subtitle:'Trendy fashion for every version of you.',eyebrow:'NEW SEASON',image_url:'/placeholder-product.svg',mobile_image_url:'/placeholder-product.svg',cta:{label:'Shop Now',href:'/shop'}}]}},
    {id:2,key:'home-promises',type:'service_strip',sort_order:20,is_active:true,content:{items:[{icon:'return',title:'7 Days',subtitle:'Easy Returns'},{icon:'shield',title:'Premium',subtitle:'Quality'},{icon:'truck',title:'COD',subtitle:'Available'},{icon:'location',title:'Pan India',subtitle:'Delivery'}]}},
    {id:3,key:'home-new-arrivals',type:'product_carousel',sort_order:30,is_active:true,title:'New Arrivals',subtitle:'Fresh styles just landed',content:{query:{sort:'newest',limit:12,in_stock:true},cta:{label:'View All',href:'/shop?sort=newest'}}},
    {id:4,key:'home-offer-banner',type:'offer_banner',sort_order:60,is_active:true,content:{eyebrow:'LIMITED TIME OFFER',title:'FLAT 30% OFF',subtitle:'Upgrade your wardrobe with selected styles.',image_url:'/placeholder-product.svg',mobile_image_url:'/placeholder-product.svg',cta:{label:'Shop Sale',href:'/shop?sort=discount'}}}
  ]}});
  ...
  if(path==='/storefront/cart/experience') return json(res,200,{success:true,data:{cart:{items:[],subtotal:0,discount_total:0,shipping_total:0,total:0,currency:'INR'}}});
  ...
  if(path==='/storefront/checkout/quote') return json(res,200,{success:true,data:{subtotal:Number(body.subtotal||999),discount_total:0,shipping_total:0,grand_total:Number(body.subtotal||999),currency:'INR',payment_methods:[{code:'cod',label:'Cash on Delivery'},{code:'razorpay',label:'Online Payment'}]}});
  if(path==='/storefront/checkout/place') return json(res,200,{success:true,data:{order:{id:state.orderId,status:'confirmed',grand_total:999}}});
  if(path==='/storefront/orders') return json(res,200,{success:true,data:{orders:[]}});
  if(path.startsWith('/storefront/orders/')) return json(res,200,{success:true,data:{order:{id:state.orderId,status:'confirmed',items:[]}}});
