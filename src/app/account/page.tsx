...
function AccountContent() {
  ...
  async function signOut() {
    ...
    try {
      ...
      // The BFF clears the HttpOnly session on logout responses and 401s.
    } finally {
  ...
  return (
    <main className="accountPage">
      ...
      <h1>My account</h1>
      <p className="muted">Manage your orders, saved styles, addresses and support from one place.</p>
      <div className="accountGrid">
        <Link href="/orders"><strong>My orders</strong><small>Track, cancel, reorder and return orders</small></Link>
        ...
        <Link href="/account/notifications"><strong>Notifications</strong><small>Offers, order updates and preferences</small></Link>
        <Link href="/account/loyalty"><strong>Wallet & loyalty</strong><small>Rewards, wallet and referral benefits</small></Link>
      </div>
      <button className="textButton" disabled={busy} onClick={signOut}>{busy ? 'Signing out…' : 'Sign out'}</button>
    </main>
  );
}
