...
export default function Account() {
  ...
  const checkSession = async () => {
    ...
    try {
      ...
      if (response.status === 401 || (response.ok && data.authenticated === false)) {
        setState('guest');
        return;
      }
  ...
  const signOut = async () => {
    try {
      ...
      // The BFF clears its HttpOnly session cookie when logout returns 401 too.
    } finally {
      clearAccessToken();
    ...
    return (
      <main className="accountPage">
        <section className="emptyState">
          ...
          <p>We could not verify your secure session right now. Your login has not been discarded.</p>
          <button className="button" type="button" onClick={() => void checkSession()}>
            Try again
          </button>
        </section>
      </main>
    ...
    return (
      <main className="accountPage">
        <section className="emptyState">
          ...
          <p>Access your orders, wishlist, addresses and support.</p>
          <Link className="button" href={`/auth/login?next=${encodeURIComponent('/account')}`}>
            Sign in with OTP
          </Link>
        </section>
      </main>
  ...
  return (
    <main className="accountPage">
      ...
      <button className="textButton" type="button" onClick={() => void signOut()}>Sign out</button>
    </main>
  );
}
