...
export default function Checkout() {
  ...
  async function validateCoupon() {
    ...
    try {
      await api('/storefront/checkout/validate', {
        method: 'POST',
        body: JSON.stringify({ coupon_code: coupon.trim().toUpperCase() || undefined }),
      });
      setMessage(coupon.trim() ? 'Coupon checked with PriyasaCore.' : 'Coupon removed.');
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Unable to validate this coupon.');
    ...
    {loading ? <div className="emptyState"><h2>Preparing your checkout…</h2></div> : <div className="checkoutGrid">
      <section>
        ...
        <div className="checkoutCard"><span className="eyebrow">STEP 2</span><h2>Offers & coupon</h2>
          <div className="coupon"><input aria-label="Coupon code" value={coupon} onChange={e => setCoupon(e.target.value.toUpperCase())} placeholder="Enter coupon code"/><button type="button" onClick={() => void validateCoupon()} disabled={validating}>{validating ? 'Checking…' : 'Apply'}</button></div>
        </div>
      ...
      <aside className="summary"><span className="eyebrow">ORDER SUMMARY</span><p>Final totals and checkout eligibility are calculated by PriyasaCore when the order is created.</p><hr/><button className="button" disabled={busy || !selected}>{busy ? (paymentMethod === 'cod' ? 'Placing order…' : 'Creating secure order…') : (paymentMethod === 'cod' ? 'Place COD order' : 'Continue to payment')}</button></aside>
    </div>}
  </main>;
}
