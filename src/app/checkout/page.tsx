...
export default function Checkout() {
  ...
  const loadQuote = useCallback(async (addressId: string, code: string) => {
    ...
    try {
      const r = await api<any>('/storefront/checkout/quote', { method: 'POST', body: JSON.stringify({ shipping_address_id: Number(addressId), coupon_code: code || '' }) });
      const nextQuote = unwrap(r);
      setQuote(nextQuote);
      return nextQuote;
    } catch (e) {
      const text = e instanceof Error ? e.message : 'Unable to calculate delivery and checkout pricing.';
      setError(text); setQuote({}); throw e;
    }
  }, []);
  ...
  const refreshQuote = useCallback(async (addressId: string, code: string) => {
    if (!addressId) return;
    setError('');
    try { await loadQuote(addressId, code); } catch { /* loadQuote exposes the API error */ }
  }, [loadQuote]);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (preferred) { const id = String(preferred.id); setSelected(id); await loadQuote(id, ''); }
      } catch (e) { setError(e instanceof Error ? e.message : 'Unable to load checkout.'); }
      finally { setLoading(false); }
    })();
  }, [loadQuote]);

  async function selectAddress(id: string) { setSelected(id); await refreshQuote(id, coupon.trim().toUpperCase()); }
  ...
  async function placeOrder() {
    ...
    if (delivery?.serviceable === false) { setError('Delivery is unavailable for this pincode.'); return; }
    if (paymentMethod === 'cod' && delivery?.cod?.eligible === false) { setError(delivery.cod.reason || 'Cash on Delivery is unavailable for this delivery.'); return; }
    setBusy(true); setError(''); setMessage('');
    try {
      const r = await api<any>('/storefront/checkout/place', { method: 'POST', body: JSON.stringify({ shipping_address_id: Number(selected), coupon_code: coupon.trim().toUpperCase(), payment_method: paymentMethod }) });
      const v = unwrap(r); const orderId = v.order?.id ?? v.id ?? v.order_id;
      ...
      <section>
        ...
        <div className="checkoutCard"><span className="eyebrow">STEP 3</span><h2>Payment method</h2>
          <div className="paymentOptions">
            ...
            <button type="button" className={`paymentChoice ${paymentMethod === 'cod' ? 'selected' : ''}`} disabled={delivery?.cod?.eligible === false} onClick={() => setPaymentMethod('cod')}><b>Cash on Delivery</b><span>{delivery?.cod?.eligible === false ? (delivery.cod.reason || 'Unavailable for this delivery') : 'Pay securely when your order is delivered'}</span></button>
          </div>
