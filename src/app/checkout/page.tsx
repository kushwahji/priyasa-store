...
function CheckoutContent() {
  ...
  async function load() {
    ...
    try {
      const [addressResponse, cartResponse] = await Promise.all([api<{ data?: Address[] }>('/storefront/addresses'), api<{ data?: Cart }>('/storefront/cart')]);
  ...
  async function validateCoupon() {
    ...
    try { await api('/storefront/checkout/validate', { method: 'POST', body: JSON.stringify({ coupon_code: code }) }); setCouponState('valid'); return true; }
    catch (e) { setCouponState('invalid'); setError(e instanceof Error ? e.message : 'Coupon could not be validated'); return false; }
  }
  ...
  async function placeOrder() {
    ...
    try {
      ...
      window.location.href = paymentMethod === 'cod' ? `/orders/${encodeURIComponent(String(orderId))}` : `/checkout/payment?order=${encodeURIComponent(String(orderId))}`;
