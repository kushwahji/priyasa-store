...
function statusValues(v: any): string[] { const values = [v?.payment_status, v?.status, v?.payment?.status, v?.payment?.payment_status]; return values.map(x => String(x ?? '').toLowerCase().trim()).filter(Boolean); }
function transactionStatuses(v: any): string[] { return Array.isArray(v?.paymentTransactions) ? v.paymentTransactions.map((x: any) => String(x?.status ?? '').toLowerCase().trim()).filter(Boolean) : []; }
function isSettled(v: any): boolean { const primary = statusValues(v); if (primary.length) return primary.some(s => ['paid', 'captured', 'success', 'successful', 'completed'].includes(s)); return transactionStatuses(v).some(s => ['paid', 'captured', 'success', 'successful', 'completed'].includes(s)); }
function isFailed(v: any): boolean { return [...statusValues(v), ...transactionStatuses(v)].some(s => ['failed', 'cancelled', 'refunded'].includes(s)); }
...
export default function PaymentPage() {
  const [orderId, setOrderId] = useState(''); const [payment, setPayment] = useState<any>(null); const [busy, setBusy] = useState(true); const [error, setError] = useState(''); const [message, setMessage] = useState('Checking payment status…');

  useEffect(() => {
    const id = new URLSearchParams(window.location.search).get('order') || ''; setOrderId(id);
    if (!id) { setError('Missing order id.'); setBusy(false); return; }
    (async () => {
      try {
        const statusResponse = await api<any>(`/storefront/orders/${encodeURIComponent(id)}`); const current = unwrap(statusResponse);
        ...
        const v = unwrap(r); const p = v.payment ?? v; setPayment(p);
        if (isSettled(v) || isSettled(p)) { window.location.assign(`/orders/${encodeURIComponent(id)}`); return; }
        const shortUrl = p.short_url ?? p.checkout_url ?? p.payment_url; if (shortUrl) { window.location.assign(shortUrl); return; }
        const razorpayOrderId = p.razorpay_order_id ?? p.provider_order_id ?? p.order_id; const amount = Number(p.amount ?? p.amount_minor ?? 0); const key = p.key_id ?? p.razorpay_key_id ?? p.key;
        ...
        const rzp = new (window as WindowWithRazorpay).Razorpay!({
          ...
          handler: async (response: RazorpayResponse) => {
            ...
            try {
              ...
              await api(`/storefront/orders/${encodeURIComponent(id)}/payment/capture`, {
                method: 'POST',
                body: JSON.stringify({
