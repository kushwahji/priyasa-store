...
export async function GET(request: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${UPSTREAM}/storefront/session`, { method: 'GET', headers: { Accept: 'application/json', Authorization: `Bearer ${token}` }, cache: 'no-store' });
    if (upstream.status === 401) {
      const response = NextResponse.json({ authenticated: false }, { status: 401, headers: { 'Cache-Control': 'no-store' } });
      clearSession(response);
      return response;
    }
    if (!upstream.ok) return NextResponse.json({ authenticated: false, message: 'Unable to verify the Priyasa session right now.' }, { status: upstream.status >= 500 ? 502 : upstream.status, headers: { 'Cache-Control': 'no-store' } });
