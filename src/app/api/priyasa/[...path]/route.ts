...
function allowedOrigin(request: NextRequest) {
  ...
  const sameOrigin = normaliseOrigin(request.nextUrl.origin);
  const configured = [process.env.NEXT_PUBLIC_STORE_URL, process.env.PRIYASA_STORE_URL]
    .filter(Boolean)
    .map((value) => normaliseOrigin(value as string))
    .filter(Boolean);
  return incoming === sameOrigin || configured.includes(incoming);
}
...
export async function ALL(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  if (route === '_session' && request.method === 'GET') {
    return NextResponse.json({ authenticated: Boolean(request.cookies.get(SESSION_COOKIE)?.value) }, { headers: { 'Cache-Control': 'no-store' } });
  }
  ...
  try {
    ...
    return NextResponse.json({ message: error instanceof Error && error.name === 'AbortError' ? 'PriyasaCore request timed out.' : 'PriyasaCore is temporarily unavailable.' }, { status: 502 });
  } finally {
