...
async function proxy(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  // The browser must talk only to this Store origin. The upstream API is never exposed to the browser,
  // so its CORS policy cannot break Store authentication or checkout.
  ...
  for (const [key, value] of request.headers) {
    ...
    if (
      ['host', 'content-length', 'connection', 'cookie', 'authorization', 'origin', 'referer', 'x-priyasa-store-proxy'].includes(lower)
    ) continue;
