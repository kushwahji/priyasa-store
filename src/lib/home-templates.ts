...
export function resolveHomeTheme(value?: string, now = new Date()): Exclude<HomeThemeKey,'auto'> {
  ...
  if(value === 'auto'){
    ...
    if(inRange(month,10,11)) return 'diwali';
    if(inRange(month,2,3)) return 'holi';
    if(inRange(month,9,10)) return 'navratri';
    if(inRange(month,6,9)) return 'monsoon';
    if(inRange(month,11,2)) return 'winter';
...
export function themeFromHome(home:any): Exclude<HomeThemeKey,'auto'> {
  ...
  const configured = section?.content?.theme || section?.content?.key || section?.content?.template;
