const UPSTREAM=(process.env.PRIYASA_API_BASE_URL||process.env.PRIYASA_API_URL||'http://localhost:8000/api/v1').replace(/\/$/,'');
...
export async function api<T>(path:string,init:RequestInit={}){
  ...
  const res=await fetch(`${base}${normalized}`,{...init,headers,cache:'no-store'});
