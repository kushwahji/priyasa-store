...
export default function FestivalTheme({ children }: Props) {
  ...
  useEffect(() => {
    ...
    void api<AnyRecord>('/storefront/home').then((payload) => {
      if (active) setThemeKey(readTheme(payload));
    }).catch(() => undefined);
    return () => { active = false; };
  }, []);
  ...
  const particles = theme.particles.map((symbol, index) => ({ symbol, index }));
  return <div className={`festivalRoot ${theme.className}`} data-festival={theme.key}>
    ...
    {theme.key !== 'default' && <div className="festivalAtmosphere" aria-hidden="true">
      <div className="festivalGlow festivalGlowA" />
      <div className="festivalGlow festivalGlowB" />
      <div className="festivalParticles">{[...particles, ...particles, ...particles].map(({ symbol, index }, i) => <span key={`${index}-${i}`} style={{ '--i': i, '--n': particles.length * 3 } as React.CSSProperties}>{symbol}</span>)}</div>
