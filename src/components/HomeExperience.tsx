...
function normalizeHome(result: AnyRecord): HomeData {
  const candidates = [result?.data?.home, result?.home, result?.data?.data, result?.data, result];
  const home = candidates.find((value) => value && typeof value === 'object' && Array.isArray(value.sections));
  if (home) return home as HomeData;
  const sections = result?.sections || result?.data?.sections || [];
  return { ...(result || {}), sections: Array.isArray(sections) ? sections : [] };
}
...
function ServiceIcon({ name }: { name?: string }) { const icon = String(name || '').toLowerCase(); return <span className="serviceIcon" aria-hidden="true">{icon === 'shield' || icon === 'secure' ? '✓' : icon === 'location' || icon === 'truck' || icon === 'delivery' ? '⌖' : icon === 'support' || icon === 'headset' ? '?' : '↩'}</span>; }
...
export default function HomeExperience() {
  ...
  async function load() {
    ...
    try {
      ...
      try { result = await api<AnyRecord>('/storefront/home'); }
      catch (firstError) {
        // Keep the Store compatible with a Core deployment exposing the canonical home resource at /home.
        result = await api<AnyRecord>('/home').catch(() => { throw firstError; });
      }
      ...
      const entries = await Promise.all(dynamic.map(async (section) => {
        ...
        if (!params.has('per_page')) params.set('per_page', String(query.limit || 12));
  ...
  return <>{sections.map((section) => {
    ...
    if (productSectionTypes.has(section.type ?? '')) { const items = (products[key] || []).map(cardProduct); if (!items.length) return null; const desktopLayout = sectionLayout(section); return <section className={`section dynamicProducts ${section.type === 'product_grid' ? 'isGrid' : ''}`} key={key}><div className="sectionHead"><div><span className="eyebrow">{section.type === 'flash_sale' ? 'LIMITED TIME' : 'PRIYASA PICKS'}</span><h2>{section.title || 'Discover more'}</h2>{section.subtitle && <p className="muted">{section.subtitle}</p>}</div>{content.cta?.href && <Link className="textLink" href={content.cta.href}>{content.cta.label || 'View all'} →</Link>}</div><div className={`homeProductRail ${layoutClass(section, 'products')} ${desktopLayout === 'stack' ? 'isStack' : ''}`}>{items.map((product) => <ProductCard key={String(product.id || product.slug)} product={product} />)}</div></section>; }
