...
const protectedPrefixes = ['/account', '/orders', '/cart', '/wishlist', '/addresses', '/checkout'];

function BrandLogo({ footer = false }: { footer?: boolean }) {
  return <Link className={`logo${footer ? ' footerLogo' : ''}`} href="/" aria-label="PRIYASA home">
    <img src="/brand/priyasa-logo.svg" alt="PRIYASA — Every You, Beautiful" />
  </Link>;
}
...
export default function StoreShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/priyasa/_session', { credentials: 'include', cache: 'no-store' })
  ...
  function submitSearch(e: React.FormEvent) {
    e.preventDefault(); const value = query.trim();
