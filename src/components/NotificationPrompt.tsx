...
function readPreferences(): Preferences { try { const value = JSON.parse(localStorage.getItem(KEY) || 'null'); return { order: value?.order !== false, cart: value?.cart !== false, offers: value?.offers !== false }; } catch { return DEFAULTS; } }
async function registerFcm(preferences: Preferences): Promise<boolean> {
  ...
  const guestId = localStorage.getItem('priyasa_guest_id') || crypto.randomUUID();
  localStorage.setItem('priyasa_guest_id', guestId);
  await api('/device/register', { method: 'POST', body: JSON.stringify({ token, device_id: `web-${token.slice(0, 20)}`, platform: 'web', device: navigator.userAgent.slice(0, 255), browser: navigator.userAgent.slice(0, 255), timezone: Intl.DateTimeFormat().resolvedOptions().timeZone, permission: Notification.permission, channel: 'fcm', delivery_target: 'browser', source: 'priyasa-store', guest_id: guestId }) });
  localStorage.setItem(KEY, JSON.stringify(preferences)); localStorage.setItem(REGISTERED, '1');
...
export default function NotificationPrompt() {
  const [visible, setVisible] = useState(false); const [busy, setBusy] = useState(false); const [preferences, setPreferences] = useState<Preferences>(DEFAULTS); const [toast, setToast] = useState<{ title: string; body: string } | null>(null);
  useEffect(() => { if (window.location.pathname !== '/') return; const prefs = readPreferences(); setPreferences(prefs); if (!('Notification' in window)) return; if (Notification.permission === 'granted') { if (localStorage.getItem(REGISTERED) !== '1') void registerFcm(prefs).catch(() => undefined); return; } if (Notification.permission === 'denied' || localStorage.getItem('priyasa_notification_prompt_dismissed') === '1') return; const timer = window.setTimeout(() => setVisible(true), 1800); return () => window.clearTimeout(timer); }, []);
  ...
  async function allow() { setBusy(true); try { const permission = await Notification.requestPermission(); if (permission !== 'granted') { localStorage.setItem('priyasa_notification_prompt_dismissed', '1'); setVisible(false); return; } await registerFcm(preferences); setVisible(false); } catch { setVisible(false); } finally { setBusy(false); } }
