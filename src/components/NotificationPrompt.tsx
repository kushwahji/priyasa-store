...
async function registerFcm(preferences: Preferences): Promise<boolean> {
  ...
  onMessage(messaging, (payload) => {
    const title = payload.notification?.title || 'PRIYASA';
    const body = payload.notification?.body || 'You have a new shopping update.';
    window.dispatchEvent(new CustomEvent('priyasa-fcm-message', { detail: { title, body } }));
  });
  return true;
}
...
export default function NotificationPrompt() {
  ...
  useEffect(() => {
    ...
    if (path.startsWith('/auth/') || path.startsWith('/checkout/payment')) return;
    ...
    const timer = window.setTimeout(() => { if (!dismissed && !alreadyGranted) setVisible(true); }, 1800);
    return () => window.clearTimeout(timer);
  }, []);
  useEffect(() => {
    const handler = (event: Event) => { const detail = (event as CustomEvent<{ title: string; body: string }>).detail; setToast(detail); window.setTimeout(() => setToast(null), 5000); };
    window.addEventListener('priyasa-fcm-message', handler);
    return () => window.removeEventListener('priyasa-fcm-message', handler);
  }, []);
  async function allow() {
    setBusy(true);
    try {
      const permission = await Notification.requestPermission();
      if (permission !== 'granted') { localStorage.setItem('priyasa_notification_prompt_dismissed', '1'); setVisible(false); return; }
      localStorage.setItem(KEY, JSON.stringify(preferences));
      await registerFcm(preferences);
      setVisible(false);
  ...
  return <aside className="notificationPrompt" role="dialog" aria-modal="false" aria-labelledby="notification-title">
    <div className="notificationPromptHead"><div className="notificationIcon">P</div><div><h3 id="notification-title">Stay Updated with PRIYASA</h3><p>Get order updates, useful cart reminders and selected shopping offers.</p></div><button className="notificationClose" onClick={dismiss} aria-label="Close">×</button></div>
    <div className="notificationReminder"><label><input type="checkbox" checked={preferences.order} onChange={e => setPreferences(v => ({ ...v, order: e.target.checked }))} /> Order & delivery updates</label><label><input type="checkbox" checked={preferences.cart} onChange={e => setPreferences(v => ({ ...v, cart: e.target.checked }))} /> Cart reminders</label><label><input type="checkbox" checked={preferences.offers} onChange={e => setPreferences(v => ({ ...v, offers: e.target.checked }))} /> Offers & new arrivals</label></div>
    <div className="notificationActions"><button className="button" disabled={busy} onClick={allow}>{busy ? 'Enabling…' : 'Allow Notifications'}</button><button className="button secondary" disabled={busy} onClick={dismiss}>Not Now</button></div>
  </aside>;
}
