...
export default function ProductActions({ variantId, sizes = [], colors = [] }: { variantId: string | number; sizes?: string[]; colors?: string[] }) {
  ...
  async function addToBag() {
    if (!variantId) return setMessage('This product variant is unavailable.');
    if (sizes.length && !size) return setMessage('Please select a size.');
    setBusy('bag'); setMessage('');
    try {
      await api('/storefront/cart/items', { method: 'POST', body: JSON.stringify({ variant_id: variantId, quantity: 1 }) });
  ...
  return <>
    ...
    <div className="pdpActions">
      <button type="button" className="button secondary" disabled={busy !== null || !variantId} onClick={toggleWishlist}>{busy === 'wishlist' ? 'SAVING…' : '♡ WISHLIST'}</button>
      <button type="button" className="button" disabled={busy !== null || !variantId} onClick={addToBag}>{busy === 'bag' ? 'ADDING…' : 'ADD TO BAG'}</button>
    </div>
